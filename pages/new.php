...
<div id="user" user-id=<?php echo "'".$_SESSION['User']."'"; ?> />
<div id="new-event">
	<form class="col s12 new-event-form" enctype="multipart/form-data" action="" method="post" id="new-event-form">
		...
		<div class="row">
			...
			<div class="input-field col s2">
				<select multiple="multiple" class="new-event-multiple-input" required id="type">
					...
					$results = getEventTypes();
					if(is_array($results) || is_object($results))
					{
						foreach ($results as $result) {
							?>
							<option value=<?php echo "".$result["Id"]."";?>><?php echo "".$result["ShortName"]."";?></option>
							<?php
						}
					}
					?>
		...
		<div class="row">
			<div class="submit field col s6 left">
				<button class="btn waves-effect waves-light" type="reset" value="reset" name="action" id="clear-form-content">Zurücksetzen
					<i class="material-icons right">undo</i>
				</button>
			</div>
			<div class="submit field col s6 right">
				<button class="btn waves-effect waves-light" type="submit" value="submit" name="action" id="submit-form-content">Veranstaltung eröffnen
