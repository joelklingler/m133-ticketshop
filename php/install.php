<?php
echo "Install script started, 0/13 Steps done.";
$username = 'root';
$password = '';
$host = 'localhost';
...
if(!$conn->connect_error)
{
	...
	{
		echo "<p>Database Deleted";
	}
	echo "<br />(0/13) Connected to localhost";
	if($conn->query($sql) === TRUE)
	{
		echo "<br />(1/13) Database '" . $database . "' created.";
		...
		if($conn->query($sql) === TRUE)
		{
			echo "<br />(2/13) Table 'Users' created.";
			$sql = 'CREATE TABLE Roles (
			Id INT(6) AUTO_INCREMENT PRIMARY KEY,
			ShortName VARCHAR(60) NOT NULL)';
			if($conn->query($sql) === TRUE)
			{
				echo "<br />(3/13) Table 'Roles' created.";
				...
				if($conn->query($sql) === TRUE)
				{
					echo "<br />(4/13) Table 'UserRoles' created";
					...
					if($conn->query($sql) === TRUE)
					{
						echo "<br />(5/13) Table 'EventType' created";
						...
						if($conn->query($sql) === TRUE)
						{
							echo "<br />(6/13) Table 'Events' created";
							...
							if($conn->query($sql) === TRUE)
							{
								echo "<br /> (7/13) Table 'Tickets' created";
								...
								if($conn->query($sql) === TRUE)
								{
									echo "<br />(7/13) Table 'EventTypes' created"								;
									...
									if($conn->query($sql) === TRUE)
									{
										echo "<br />(8/13) Test User's created.";
        								...
        								if($conn->query($sql) === TRUE)
        								{
        									echo "<br />(9/13) Test Roles created";
        									...
        									if($conn->query($sql) === TRUE)
        									{
        										...
        										if($conn->query($sql) === TRUE)
        										{
													...
													if($conn->query($sql) === TRUE)
													{
														echo "<br />(12/13) Table 'Orders' created";
														// Create Test-Events
														...
														if($conn->query($sql) === TRUE)
														{
															echo '<br />(13/13) Testdata created</p>';
														}
														else
														{
															echo $conn->error;
